...
/**
   ...
   */(Renderer),_selectors=(obj=_selectors)&&obj.__esModule?obj:{default:obj};return _exports.default=class{constructor(baseSelector){this.baseElement=document.querySelector(baseSelector)}async init(){const modalFooter=document.querySelector(_selectors.default.modalFooter),backButton=modalFooter.querySelector('[data-action="back"]'),generateButton=modalFooter.querySelector('[data-action="generate"]'),showPromptButton=this.baseElement.querySelector('[data-action="showprompt"]');backButton&&backButton.addEventListener("click",(async()=>{await Renderer.renderStart(_constants.constants.modalModes.selection)})),generateButton&&generateButton.addEventListener("click",(async()=>{await Renderer.renderLoading(),await new Promise((resolve=>setTimeout(resolve,1e3))),await Renderer.renderSuggestion("BLINDTEXT")})),showPromptButton&&showPromptButton.addEventListener("click",(async()=>{const expanded=showPromptButton.dataset.hasOwnProperty("expanded")&&showPromptButton.dataset.expanded.length>0;await Renderer.renderSummarize(!expanded)}))}},_exports.default}));
